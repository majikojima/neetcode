...
"""
動的計画法（Dynamic Programming、DPと略されることもある）は、最適化の問題を解くための手法の一つで、大きな問題をより小さな部分問題に分解して、それぞれの部分問題の答えを計算し、その結果を利用して元の問題の答えを求めるという方法です。動的計画法は、特に再帰的な構造を持つ問題に対して有効です。

動的計画法の主な特徴として以下の2つが挙げられます：

1. **最適部分構造**：大きな問題が小さな部分問題に分解でき、それぞれの部分問題の最適な解が合わせることで全体の最適な解が得られる構造を持っている。

2. **重複する部分問題**：同じ部分問題が何度も計算されるような場合に、その結果を保存しておくことで計算時間を大幅に削減することができる。

動的計画法の基本的なステップは以下の通りです：

1. 大きな問題を小さな部分問題に分解する。
2. 最小の部分問題から答えを計算していき、それをメモ化テーブル（通常は配列やリストとして実装される）に保存する。
3. メモ化テーブルの値を利用して、次の部分問題の答えを計算する。
4. 最終的に全体の問題の答えを得る。

例えば、フィボナッチ数列の計算を動的計画法を用いて行う場合、次のようなコードになります：
...
上記のコードでは、`fib`というメモ化テーブルを用いて、計算したフィボナッチ数を保存しています。これにより、同じ数値のフィボナッチ数を再計算することなく、効率的に結果を得ることができます。
"""
